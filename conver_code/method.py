...
class DeviceAgent(object):
    ...
    def is_converged(self):
        """
        判断WoLF-PHC算法是否收敛
        :param policy_history: 智能体在训练过程中的策略历史记录，为一个列表，每个元素为一个策略向量
        :param threshold: 策略变化的阈值，当两个策略向量之间的欧氏距离小于该阈值时认为策略已经收敛
        :param window_size: 计算策略变化的滑动窗口大小，取最近的window_size个策略向量计算策略变化
        :return: 如果策略已经收敛，返回True，否则返回False
        """
        if len(self.policy_history) < self.window_size:
            return False
        
        last_policies = self.policy_history[-self.window_size:]
        distances = [np.linalg.norm(np.array(last_policies[i]) - np.array(last_policies[i-1])) for i in range(1, len(last_policies))]
