...
class Device(object):
    ...
    def __init__(self, id, loc: list[int], f_c=None, f_p=None, task: list[int]=None) -> None:
        self.id = id
        ...
        x_index = int(
            (self.loc[0]+WIDTH/DIVIDE_WIDTH)/(WIDTH/DIVIDE_WIDTH))
        y_index = int(
            (self.loc[1]+WIDTH/DIVIDE_WIDTH)/(WIDTH/DIVIDE_WIDTH))
        self.region = (y_index-1)*DIVIDE_WIDTH+x_index
    ...
    def get_trans_rate(self, ap_id):
        for aid, trans_rate, _ in self.access_aps:
            if aid == ap_id:
                return trans_rate
        raise ValueError
...
class AP(object):
    ...
    def __init__(self, id, total_b=None, total_f=None, loc=None) -> None:
        self.id = id
        self.loc = loc  # location (x, y)
        self.total_b = total_b
        self.total_f = total_f

        x_index = int((self.loc[0]+WIDTH/DIVIDE_WIDTH)/(WIDTH/DIVIDE_WIDTH))
        y_index = int((self.loc[1]+WIDTH/DIVIDE_WIDTH)/(WIDTH/DIVIDE_WIDTH))
        self.region = (y_index-1)*DIVIDE_WIDTH+x_index

        self.cover_region = [self.region]
        if self.region-4 >= 0:
            self.cover_region.append(self.region-4)
        if self.region+4 <= (DIVIDE_WIDTH*DIVIDE_WIDTH):
            self.cover_region.append(self.region+4)
        if self.region-1 >= 0:
            self.cover_region.append(self.region-1)
        if self.region+1 <= (DIVIDE_WIDTH*DIVIDE_WIDTH):
            self.cover_region.append(self.region+1)
...
class Env(object):
    ...
    def get_access_aps(self):
        for device in self.devices:
            for ap in self.aps:
                if device.region in ap.cover_region:
                    d_im = math.sqrt(
